...
function verifyStockAvailability(productType, qty) {
  ...
  //else return true;
  if (availableStock === 0 || qty > availableStock) return false; // Correção: Verifica se a quantidade desejada é maior que o estoque disponível
  else return true;
}
...
function calculateTotalPrice(products) {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    total += products[i].price * products[i].quantity; // Correção: Multiplica preço pela quantidade
  }
  return total;
}
