...
export class TurmaView {
    ...
    constructor(private selector: string, turmaService: TurmaService) {
    ...
    inicializarFormulario() {
        ...
        if (!btnSalvar) throw new Error("Botão de salvar não encontrado");
    
        btnSalvar.addEventListener('click', () => {
            const nomeCompletoInput = document.getElementById("nome") as HTMLInputElement;
    ...
    renderizarListaAlunos() {
        ...
        if (!listaAlunosElement) throw new Error("Lista de alunos não encontrada");
    ...
    editarAluno(id: number) {
        ...
        if (!aluno) throw new Error("Aluno não encontrado para edição.");
    ...
    renderizarDetalhesTurma() {
        ...
        if (!detalhesTurmaElement) return;
    ...
    update(turma: Turma) {
        ...
        let alunosHtml = turma.getAlunos().map(aluno =>
            `<tr>
                <td>${aluno.nomeCompleto}</td>
                <td>${aluno.idade}</td>
                <td>${aluno.altura}m</td>
