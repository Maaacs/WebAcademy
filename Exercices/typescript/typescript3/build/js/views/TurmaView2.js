...
export class TurmaView {
    ...
    inicializarFormulario() {
        ...
        btnSalvar.addEventListener('click', () => {
            ...
            if (!nomeCompletoInput.value.trim() || !idadeInput.value.trim() || !alturaInput.value.trim() || !pesoInput.value.trim()) {
                ...
                return; // Impede a adição do aluno se algum campo estiver vazio
            }
            const novoAluno = new Aluno(Date.now(), // Supondo que você está usando um timestamp para o ID
            nomeCompletoInput.value, parseInt(idadeInput.value), parseFloat(alturaInput.value), parseFloat(pesoInput.value));
    ...
    editarAluno(id) {
        ...
        this.turmaService.editarAluno(new Aluno(id, novoNome, parseInt(novaIdade), parseFloat(novaAltura), parseFloat(novoPeso)));
        this.renderizarListaAlunos();
        this.renderizarDetalhesTurma();
    }
    removerAluno(id) {
