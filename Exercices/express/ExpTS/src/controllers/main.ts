import { Request, Response } from 'express';
import { LoremIpsum } from 'lorem-ipsum';

const loremInstance = new LoremIpsum();

const inicial = (req: Request, res: Response): void => {
  res.render('main/inicial');
};

const sobre = (req: Request, res: Response): void => {
  res.render('main/sobre');
};

const lorem = (req: Request, res: Response): void => {
  const num = parseInt(req.params.num, 10);
  let paragraphs = '';

  for (let i = 0; i < num; i++) {
    const numberOfSentences = Math.floor(Math.random() * (8 - 3 + 1)) + 3;
    paragraphs += `<p>${loremInstance.generateSentences(numberOfSentences)}</p>`;
  }

  if (req.headers['x-requested-with'] === 'XMLHttpRequest') {
    res.send(paragraphs); // Envie apenas o texto
  } else {
    // Caso contrário, renderize a página completa
    res.render('main/lorem', {
      loremParagraphs: paragraphs,
    });
  }
};

const hb1 = (req: Request, res: Response): void => {
  res.render('main/hb1', {
    mensagem: 'Olá, você está aprendendo Express + HBS!',
  });
};

const hb2 = (req: Request, res: Response): void => {
  res.render('main/hb2', {
    poweredByNodejs: true,
    name: 'Express',
    type: 'Framework',
  });
};

const hb3 = (req: Request, res: Response): void => {
  const profes = [
    { nome: 'David Fernandes', sala: 1238 },
    { nome: 'Horácio Fernandes', sala: 1233 },
    { nome: 'Edleno Moura', sala: 1236 },
    { nome: 'Elaine Harada', sala: 1231 },
  ];
  res.render('main/hb3', { profes });
};

const hb4 = (req: Request, res: Response): void => {
  const technologies = [
    { name: 'Express', type: 'Framework', poweredByNodejs: true },
    { name: 'Laravel', type: 'Framework', poweredByNodejs: false },
    { name: 'React', type: 'Library', poweredByNodejs: true },
    { name: 'Handlebars', type: 'Engine View', poweredByNodejs: true },
    { name: 'Django', type: 'Framework', poweredByNodejs: false },
    { name: 'Docker', type: 'Virtualization', poweredByNodejs: false },
    { name: 'Sequelize', type: 'ORM tool', poweredByNodejs: true },
  ];

  res.render('main/hb4', { technologies });
};

export default { inicial, sobre, lorem, hb1, hb2, hb3, hb4 };
